...
export default function Home() {
  ...
  const fetchBuilding = useCallback(async () => {
    try {
      ...
      // Récupérer les passagers qui attendent
      if (data.id) {
        const passengersResponse = await fetch(`/api/passenger?buildingId=${data.id}&status=waiting`);
        if (passengersResponse.ok) {
          const passengersData = await passengersResponse.json();
          setWaitingPassengers(passengersData);
        }
      }
    } catch (error) {
